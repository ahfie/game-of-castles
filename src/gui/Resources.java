...
/**
 * Diese Klasse verwaltet die Resourcen des Spiels, darunter beispielsweise
...
public class Resources {
	 ...
	 * @author Liang Zhao
	 */
	private BufferedImage joker, jokerDeactivated;
	...
	public boolean load() {
		...
		try {
			...
			jokerDeactivated = loadImage("joker_deactivated.png");
	...
	private void loadScoreEntries() {
		List<ScoreEntry> readEntries;
		try {
			BufferedReader br;
			br = new BufferedReader(new FileReader("highscores.txt"));
			readEntries = br.lines().filter(n -> n.split(";").length == 4).map(n -> ScoreEntry.read(n))
		...
		try {
			Collections.sort(readEntries);
			Collections.reverse(readEntries);
			for (ScoreEntry scoreEntry : readEntries) {
				addScoreEntry(scoreEntry);
			}
		} catch (NullPointerException e) {
			JOptionPane.showMessageDialog(null, "Highscoredaten enthält ungultige Zeilen",
					"Fehler beim Laden der Highscore", JOptionPane.ERROR_MESSAGE);
			// filter invalid data, salvage valid data
			List<ScoreEntry> filteredEntries = readEntries.stream().filter(n -> n != null).collect(Collectors.toList());
			Collections.sort(filteredEntries);
			Collections.reverse(filteredEntries);
			for (ScoreEntry scoreEntry : filteredEntries) {
				addScoreEntry(scoreEntry);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private BufferedImage[] loadDices() throws IOException {
		...
		if (diceSizeW != diceSizeH)
			System.out.println("Invalid dice dimensions for resource: dice.png. Expected 3x2 dices, got dimensions: "
					+ diceImage.getWidth() + "x" + diceImage.getHeight());
		...
		for (int x = 0; x < 2; x++) {
			for (int y = 0; y < 3; y++) {
				...
				gr.drawImage(diceImage, 0, 0, diceSize, diceSize, diceSize * y, diceSize * x, diceSize * y + diceSize,
						diceSize * x + diceSize, null);
				gr.dispose();
			}
		}
	...
	/**
	 * @author Liang Zhao
	 * @return buffered image of the joker-deactivated icon
	 */
	public BufferedImage getJokerIconDeactivated() {
		return this.jokerDeactivated;
	}

	/**
	 * @author Liang Zhao
	 * @return buffered image of the joker icon
	 */
	public BufferedImage getJokerIcon() {
		return this.joker;
	}
