...
public class MapPanel extends JScrollPane {
		...
		public void mouseDragged(MouseEvent e) {
			...
			if (oldView != null && mousePos != null && !mousePos.equals(e.getPoint())) {
				...
				if (getWidth() < imagePanel.getWidth() || getHeight() < imagePanel.getHeight()) {
					...
					// don't update view position, if you cannot move in this direction (e.g. vp.w
					// >= img.w)
			...
			if (e.getButton() == MouseEvent.BUTTON1) {
				...
				if (selectedCastle != null && canPerformAction()) {
					...
					if (canChooseCastle()) {
						 ...
						 * @author Liang Zhao
						 */
						Rectangle iconJoker = getBoundsJokerIcon(castlePos);
						 ...
						 * @author Liang Zhao
						 */
						else if (iconJoker.contains(mousePos)) {
							if (currentPlayer.getJokerPoints() > 0 && game.getRound() > 6) {
								int jokerReward = (game.getMap().getCastles().size()
										- currentPlayer.getCastles(game).size()) / 2;
								game.addJokerTroops(currentPlayer, selectedCastle, jokerReward);
								gameView.updateStats();
							}
						}
					...
					if (currentAction != Action.NONE) {
						if (lastAction != currentAction) {
							pathFinding = new PathFinding(game.getMap().getGraph(), selectedCastle, currentAction,
									currentPlayer);
							pathFinding.run();
						}
				...
				if (selectNew) {
					...
					if (nextCastle == null || nextCastle == selectedCastle || currentAction == Action.NONE) {
						...
						NumberDialog nd = new NumberDialog("Wie viele Truppen möchtest du verschieben?", 1,
								selectedCastle.getTroopCount() - 1, 1);
					...
					} else if (currentAction == Action.ATTACKING && pathFinding.getPath(nextCastle) != null
							&& nextCastle.getOwner() != selectedCastle.getOwner()) {
						NumberDialog nd = new NumberDialog("Mit wie vielen Truppen möchtest du angreifen?", 1,
								selectedCastle.getTroopCount(), selectedCastle.getTroopCount() - 1);
						if (nd.showDialog(MapPanel.this)) {
	...
	private boolean canChooseCastle() {
		...
		return game.getCurrentPlayer() instanceof Human && game.getCurrentPlayer().getRemainingTroops() > 0
				&& game.getRound() == 1 && selectedCastle.getOwner() == null;
	}
	...
	private boolean canAttack() {
		...
		return game.getCurrentPlayer() instanceof Human && selectedCastle.getOwner() == game.getCurrentPlayer()
	 ...
	 * @return
	 */
	private Rectangle getBoundsJokerIcon(Point castlePos) {
		Rectangle jokerIcon = getBoundsSwordsIcon(castlePos);
		jokerIcon.x += ICON_SIZE + 2;
		return jokerIcon;
	}
	...
	class ImagePanel extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			if (map != null) {
				...
				/**
				 * @author Jonathan Joewono
				 */
				if (game.getGoal() instanceof KingGoal) {
					if (hill == null)
						hill = ((KingGoal) game.getGoal()).getHill();
					if (hill != null) {
						int x = hill.getLocationOnMap().x;
						int y = hill.getLocationOnMap().y;
						double r = (CASTLE_SIZE + 20);
						x += offset.x - 10;
						y += offset.y - 10;
			...
			if (showConnections) {
				for (Edge<Castle> edge : map.getEdges()) {
					...
					g2.draw(new Line2D.Float(p1.x + CASTLE_SIZE / 2.0f, p1.y + CASTLE_SIZE / 2.0f,
							p2.x + CASTLE_SIZE / 2.0f, p2.y + CASTLE_SIZE / 2.0f));
					g2.setStroke(new BasicStroke(1));
				}
			}
			...
			if (currentAction != Action.NONE && targetCastle != null && highlightedEdges != null
					&& canPerformAction()) {
				BufferedImage icon = (currentAction == Action.ATTACKING ? resources.getSwordsIcon()
						: resources.getArrowIcon());
			...
			if (selectedCastle != null) {
				...
				if (canPerformAction()) {
					if (canChooseCastle()) {
						...
						BufferedImage plusIcon = hasTroops ? resources.getPlusIcon()
								: resources.getPlusIconDeactivated();
						BufferedImage swordsIcon = resources.getSwordsIcon();
						BufferedImage arrowIcon = canMove ? resources.getArrowIcon()
								: resources.getArrowIconDeactivated();

						/**
						 * @author Liang Zhao
						 */
						boolean hasJokerPoints = game.getCurrentPlayer().getJokerPoints() > 0;
						BufferedImage jokerIcon = hasJokerPoints ? resources.getJokerIcon()
								: resources.getJokerIconDeactivated();
						 ...
						 * @author Liang Zhao
						 */
						BufferedImage icons[] = { plusIcon, arrowIcon, swordsIcon, jokerIcon };
