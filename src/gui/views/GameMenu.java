...
public class GameMenu extends View {
	...
	NumberChooser playerCount;
	JComboBox<?> mapSize;
	JComboBox<?> goal;
	JComponent[][] playerConfig;
	...
	protected void onInit() {
		...
		for (int i = 0; i < GameConstants.MAX_PLAYERS; i++) {
			playerConfig[i] = new JComponent[] { createLabel(String.format("%d.", i + 1), 16),
					...
					new JTextField(String.format("Spieler %d", i + 1)), new JComboBox<>(playerTypes) };
		...
		if (actionEvent.getSource() == btnBack)
			...
			try {
				...
				for (int i = 0; i < playerCount; i++) {
					...
					if (name.isEmpty()) {
						showErrorMessage(
								String.format("Bitte geben Sie einen gültigen Namen für Spieler %d an.", i + 1),
								"Ungültige Eingaben");
						return;
					}
					...
					if (playerType < 0 || playerType >= GameConstants.PLAYER_TYPES.length) {
						showErrorMessage(
								String.format("Bitte geben Sie einen gültigen Spielertyp für Spieler %d an.", i + 1),
								"Ungültige Eingaben");
						return;
					}
					...
					if (player == null) {
						showErrorMessage(String.format("Fehler beim Erstellen von Spieler %d", i + 1),
								"Unbekannter Fehler");
						return;
					}
				...
				// pop-up mit erklärung
				JOptionPane.showMessageDialog(getWindow(), goal.getExplain(), goal.getName(),
