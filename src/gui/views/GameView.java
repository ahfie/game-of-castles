...
import game.goals.KillModusGoal;
...
public class GameView extends View implements GameInterface {
				...
				if (game.getCurrentPlayer().getRemainingTroops() > 0) {
					if (game.getRound() == 1) {
						...
						JOptionPane.showMessageDialog(this,
								String.format("Du musst noch %s auswählen.",
						...
						/**
						 * modified to fulfill the requirement
						 * 
						 * @author Liang Zhao
						 */
						if (game.getGoal() instanceof KillModusGoal) {
							JOptionPane.showMessageDialog(this,
									String.format(
											"Du hast noch unverteilte Truppen.\nAlle Truppe muss verteilt werden."),
									"Abbrechen", JOptionPane.WARNING_MESSAGE);
							return;
						} else {
							int choice = JOptionPane.showOptionDialog(this,
									"Du hast noch unverteilte Truppen.\nBist du dir sicher, dass du die Runde beenden möchtest?",
									"Runde beenden?", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE, null,
									new String[] { "Runde beenden", "Abbrechen" }, "Runde beenden");
	...
	public void updateStats() {
		...
		try {
			...
			for (Player p : game.getPlayers()) {
				...
				doc.insertString(doc.getLength(),
						String.format(":\t%d\t%d\t%d\n", p.getPoints(), p.getNumRegions(game), p.getRemainingTroops()),
						null);
			}
	...
	public void onNextTurn(Player currentPlayer, int troopsGot, boolean human) {
		if (game.getRound() == 7 && game.getGoal() instanceof ConquerGoalJoker) {
			this.logLine("Joker ist jetzt verfügbar!");
			JOptionPane.showMessageDialog(getWindow(),
					"Joker ist jetzt verfügbar!\nKlicken Sie das Jokerbutton um Bonustruppen einzusetzen!",
					"Joker freigeschaltet!", JOptionPane.INFORMATION_MESSAGE);
		}
	...
	public void onGameOver(Player winner) {
		this.logLine(game.getGoal().getEndMessage(winner), winner);
		if (winner == null) {
	...
	public void onGameStarted(Game game) {
		...
		this.logLine(game.getGoal().getExplain());
		this.updateStats();
