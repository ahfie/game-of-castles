...
import game.goals.ConquerGoalJoker;
import game.goals.RecruitGoal;
...
public class Game {
	...
	public void start(GameInterface gameInterface) {
		...
		/**
		 * set joker points to each player in different map size
		 * 
		 * @author Liang Zhao
		 */
		if (this.goal instanceof ConquerGoalJoker) {
			for (Player player : this.players) {
//				player.setJokerPoints(this.mapSize.ordinal() + 1);
				if (this.mapSize == MapSize.SMALL) {
					player.setJokerPoints(1);
				}
				if (this.mapSize == MapSize.MEDIUM) {
					player.setJokerPoints(1);
				}
				if (this.mapSize == MapSize.LARGE) {
					player.setJokerPoints(1);
				}
			}
		} else {
			for (Player player : this.players) {
				player.setJokerPoints(0);
			}
		}
	...
	/**
	 * modified to fulfill the requirement for new missions
	 * 
	 * @author Liang Zhao
	...
	public void doAttack(Castle attackerCastle, Castle defenderCastle, int[] rollAttacker, int[] rollDefender) {
		...
		Integer[] rollAttackerSorted = Arrays.stream(rollAttacker).boxed().sorted(Comparator.reverseOrder())
				.toArray(Integer[]::new);
		Integer[] rollDefenderSorted = Arrays.stream(rollDefender).boxed().sorted(Comparator.reverseOrder())
				.toArray(Integer[]::new);
		...
		for (int i = 0; i < Math.min(rollAttacker.length, rollDefender.length); i++) {
			if (rollAttackerSorted[i] > rollDefenderSorted[i]) {
				 ...
				 * @author Liang Zhao
				 */
				addNumDefeatedTroops(attacker, 1);
				 ...
				 * @author Liang Zhao
				 */
				addNumDefeatedTroops(defender, 1);
	 ...
	 * @param count
	 */
	public void addJokerTroops(Player player, Castle castle, int count) {
		if (count < 1 || castle.getOwner() != player)
			return;
		castle.addTroops(count);
		player.removeJokerPoints(1);
	}
	...
	/**
	 * add numDefeatedTroops troops to the numDefeatedTroops of the player
	 * 
	 * @author Liang Zhao
	 * @param player
	 * @param numDefeatedTroops
	 */
	public void addNumDefeatedTroops(Player player, int numDefeatedTroops) {
		player.addNumDefeatedTroops(numDefeatedTroops);
		// System.out.println("Player: " + player.getName() + " " + "numDefeatedTroops:
		// " + player.getNumDefeatedTroops());
	}
	...
	public void nextTurn() {
		...
		if (round == 1) {
			...
			/**
			 * Pro Spieler werden maximal 7 Burgen für Spielfeldgröße Klein, 14 Burgen für
			 * Mittel und 21 Burgen für Groß generiert
			 * 
			 * @author Kenneth Kartono
			 */
			int mapSizeMultiplier = this.mapSize.ordinal() + 1;
			while (addTroops + currentPlayer.getCastles(this).size() > GameConstants.CASTLES_NUMBER_MULTIPLIER
					* mapSizeMultiplier) {
				addTroops--;
			}
		} else {
		 ...
		 * @author Liang Zhao
		 */
		if (goal instanceof RecruitGoal)
			this.currentPlayer.addNumRecruitTroops(addTroops);
