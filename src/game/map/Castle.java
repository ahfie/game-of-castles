...
/**
 * Diese Klasse representiert eine Burg. Jede Burg hat Koordinaten auf der Karte
 * und einen Namen. Falls die Burg einen Besitzer hat, hat sie auch eine Anzahl
 * von zugewiesenen Truppen. Die Burg kann weiterhin Teil eines Königreichs
 * sein.
 */
public class Castle {
	...
	/**
	 * Truppen von dieser Burg zur angegebenen Burg bewegen. Dies funktioniert nur,
	 * wenn die Besitzer übereinstimmen und bei der aktuellen Burg mindestens eine
	 * Truppe übrig bleibt
	 * 
	 * @param target
	 * @param troops
	...
	/**
	 * Gibt den Burg-Typen zurück. Falls die Burg einem Königreich angehört, wird
	 * der Typ des Königreichs zurückgegeben, ansonsten 0
	 * 
	...
	/**
	 * Return a String with attributes of a castle. Used for debug purposes
	 * @author Jonathan Geraldi Joewono
	 * @return all attributes of a castle in a String
	 */
	@Override
	public String toString() {
		Optional<Player> ownerOpt = Optional.ofNullable(owner);
		Optional<Kingdom> kingdomOpt = Optional.ofNullable(kingdom);
		return new StringBuilder().append(name).append(" of Kingdom ")
				.append(kingdomOpt.orElseGet(() -> new Kingdom(6)).getType()).append(": ")
				.append(ownerOpt.orElseGet(() -> new Human("No Owner", Color.WHITE)).getName()).append(", Location: ")
				.append(location.x).append(", ").append(location.y).append(", Troops: ").append(troopCount).toString();
	}
}
