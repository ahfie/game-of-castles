...
public class PathFinding extends GraphAlgorithm<Castle> {
	...
	protected boolean isPassable(Edge<Castle> edge) {
		...
		if (action == MapPanel.Action.ATTACKING) {
			return castleA.getOwner() != null && castleB.getOwner() != null && castleA.getOwner() != castleB.getOwner()
					&& (getAlgorithmNodes().get(edge.getNodeA()).getValue() == 0
							|| getAlgorithmNodes().get(edge.getNodeB()).getValue() == 0);// added
